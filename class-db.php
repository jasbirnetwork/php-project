...
class DB {
    ...
    private $dbPassword = "Randhawa@1.";
    private $dbName     = "php_moodle";
    ...
    public function insert_new_meeting($data) {
        $this->db->query("INSERT INTO meetings(url, password, topic) VALUES('$data->join_url', '$data->password', '$data->topic')");
    }

    public function get_new_meeting($id) {

        $sql = $this->db->query("SELECT url, password, topic FROM meetings WHERE id = '$id'");
        $result = $sql->fetch_assoc();
        return $result;
    }
}